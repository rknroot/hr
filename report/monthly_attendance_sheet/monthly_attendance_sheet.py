...
def add_data(employee_map, att_map, filters, holiday_map, conditions, default_holiday_list, leave_list=None):
	...
	for emp in employee_map:
		...
		total_p = total_a = total_l = total_h = total_um = in_time = 0.0
		emp_status_map = []
		...
		if filters.summarized_view:
			row += [total_p, total_l, total_a, total_h, total_um, in_time]
		...
		if filters.summarized_view:
			...
			emp_check = frappe.db.sql("""select employee_name, time, exit_date_time from `tabEmployee Checkin`""",as_dict=1)
			for ec in emp_check:
				if ec.employee_name:
					row.append(ec.exit_date_time)
...
def get_columns(filters):
	...
	if filters.summarized_view:
		columns += [_("Total Present") + ":Float:120", _("Total Leaves") + ":Float:120",  _("Total Absent") + ":Float:120", _("Total Holidays") + ":Float:120", _("Unmarked Days")+ ":Float:120", _("In Time")+ ":Datetime:6"]
	return columns, days
