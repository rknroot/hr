...
class LeavePolicyAssignment(Document):
	...
	def grant_leave_alloc_for_employee(self):
		if self.leaves_allocated:
			...
			frappe.msgprint('leave_type_details '+str(leave_type_details))
			...
			for leave_policy_detail in leave_policy.leave_policy_details:
				if not leave_type_details.get(leave_policy_detail.leave_type).is_lwp:
					...
					frappe.msgprint('leave allocation '+str(leave_allocation) + ' new leaves allocated '+str(new_leaves_allocated))
	...
	def create_leave_allocation(self, leave_type, new_leaves_allocated, leave_type_details, date_of_joining):
		...
		frappe.msgprint('new_leaves_allocated '+str(new_leaves_allocated))
	...
	def get_new_leaves(self, leave_type, new_leaves_allocated, leave_type_details, date_of_joining):
		...
		# Calculate leaves at pro-rata basis for employees joining after the beginning of the given leave period
			...
			frappe.msgprint('in elif '+str(getdate(date_of_joining)))
			remaining_period = ((date_diff(self.effective_to, date_of_joining) + 1) / (date_diff(self.effective_to, self.effective_from) + 1))
			frappe.msgprint('remaining period '+str(remaining_period))
			new_leaves_allocated = ceil(new_leaves_allocated * remaining_period)
			frappe.msgprint('new leavessss '+str(new_leaves_allocated))
