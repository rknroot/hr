...
@frappe.whitelist()
def yearly_leave_creation():
	###On Every Year starting personal leave will be allocated to the employee.
	starting_day_of_current_year = datetime.now().date().replace(month=1, day=1)
	print(str(starting_day_of_current_year))
	dt = today()
	print('dt '+str(dt))
	if dt == starting_day_of_current_year:
		act_employee = frappe.db.sql(""" select date_of_joining, company, status, name, employee_name, department from `tabEmployee` 
				where status=%s """, ("Active"), as_dict=True)
		
		for i in act_employee:
			la = frappe.new_doc("Leave Allocation")
			la.employee = i.name
			la.from_date = i.date_of_joining
			la.leave_type = "Personal Leave"
			#end_date = datetime.now().date.replace(month=12,day=31)
			#now = getdate()
			last_date = date(date.today().year, 12, 31)
			#last_date = get_year_ending(now)
			la.to_date = last_date
			#frappe.msgprint("end date "+str(last_date))
			la.company = i.company
			pd = frappe.db.get_single_value('Leave Management Settings', 'personal_leave')
			la.new_leaves_allocated = pd
			la.save(ignore_permissions=True)
			la.submit()

@frappe.whitelist()
def earned_leave_creation():
	from collections import defaultdict

	###Additional Leave allocated on every year completion based on date_of_joining of employee
	leave_ern = frappe.db.multisql({
		"mariadb": """
		...
		"postgres": """
			...
			AND
				"status" = 'Active'
		""",
	}, dict(today=today()), as_dict=1, debug=True)

	print('leave ern '+str(leave_ern))			
	for i in leave_ern:
		...
		#frappe.msgprint("end date "+str(last_date))
		la.company = i.company
		pd = frappe.db.get_single_value('Leave Management Settings', 'additional_leave')
		la.new_leaves_allocatedq = pd
		la.save(ignore_permissions=True)
		la.submit()

def validate_to_date(self):
		#td = getdate(self.from_date) + timedelta(83)
		dd = getdate(self.from_date) + timedelta(85-1)
		d = date_diff(dd, self.from_date)
		self.to_date = dd