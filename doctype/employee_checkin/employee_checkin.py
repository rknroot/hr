...
class EmployeeCheckin(Document):
	def validate(self):
		...
		self.fetch_shift()
		###Send Notification while employee check in and check out on holidays including weekend
		self.send_notification_on_holidays()

		if self.log_in == '1' and not self.entry_date_time:
				frappe.throw('Entry Date Time is mandatory.')


		if self.log_out == '1' and not self.exit_dt_time:
				frappe.throw('Exit Date Time is mandatory.')

	def send_notification_on_holidays(self):
		holiday_date = []
		
		if self.employee and self.check_in_date:
			holiday_list = frappe.db.get_value("Employee", self.employee, 'holiday_list')
			holiday_val = frappe.db.sql("""select holiday_date, description from `tabHoliday` where parent = %s""",(holiday_list),as_dict=1)
			for i in holiday_val:
				if (getdate(self.check_in_date) == getdate(i.holiday_date)) and self.log_type == 'IN':
					subj = 'Employee Check IN on Holidays Notification '
					notification_message = '{0} has Logged in on holidays - <a href="desk#Form/Employee Checkin/{1}" target="_blank">{2}</a> \
						at {3}.'.format(self.employee_name, self.name, self.name, self.entry_date_time)

					#for receiver in mail:
					mail = frappe.get_all("Has Role",filters={"role": "HR Manager", "parenttype": "User"},fields="parent")
					for n in mail:
						if not n.parent == 'Administrator':
							frappe.sendmail(n.parent,subject=subj,\
								message = notification_message)

				elif (getdate(self.check_in_date) == getdate(i.holiday_date)) and self.log_type == 'OUT':
					subj = 'Employee Check OUT on Holidays Notification '
					notification_message = '{0} has Logged OUT on holidays - <a href="desk#Form/Employee Checkin/{1}" target="_blank">{2}</a> \
						at {3}.'.format(self.employee_name, self.name, self.name, self.exit_dt_time)
